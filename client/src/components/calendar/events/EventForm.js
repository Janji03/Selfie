...
const EventForm = ({ initialData, onSubmit, isEditMode }) => {
  ...
  const validateForm = () => {
    const newErrors = {};
    const { title, startDate, startTime, endDate, endTime, allDay, location, description } = formData;
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      if (!adjustedFormData.title || adjustedFormData.title.trim().length === 0) {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'startTime') {
      ...
      const updatedValue = `${newHours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            style={{ textDecoration: new Date(formData.endDate) < new Date(formData.startDate) ? 'line-through' : 'none' }}
          />
        ...
        {!formData.allDay && (
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              style={{ textDecoration: formData.startDate === formData.endDate && formData.endTime <= formData.startTime ? 'line-through' : 'none' }}
            />
        ...
        <button type="submit">{isEditMode ? 'Save Changes' : 'Add Event'}</button>
        {isEditMode && <button type="button" onClick={handleResetChanges}>Reset changes</button>}
      </form>
    </div>
  );
};
