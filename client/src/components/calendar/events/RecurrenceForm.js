...
const RecurrenceForm = ({ formData, setFormData, handleChange }) => {
  const { calculateEndDateRecurrence } = RecurrenceHandler();
    ...
    useEffect(() => {
      if (formData.startDate && formData.recurrence.type) {
          const newEndDate = calculateEndDateRecurrence(
              formData.startDate, 
              formData.recurrence.type === 'CUSTOM' ? formData.recurrence.frequency : formData.recurrence.type
          );
    ...
    }, [formData.startDate, formData.recurrence.type, formData.recurrence.frequency, formData.recurrence.interval]);
      ...
      return (
        <div>
            <div>
              ...
              <select name="recurrence.type" value={formData.recurrence.type} onChange={handleChange}>
              ...
              <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    min="1"
                  />
                  <span>{formatRecurrenceFrequency(formData.recurrence.frequency, formData.recurrence.interval || 1)}</span>
                </div>
                    ...
                    {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map(day => (
                      <div key={day}>
                        <label>
                          <input
                            type="checkbox"
                            checked={formData.recurrence.daysOfWeek.includes(dayMapping[day])}
                    ...
                    <div>
                      ...
                      <label>
                        <input
                          ...
                          checked={formData.recurrence.monthlyType === 'weekdayOccurrences'}
                          onChange={() => setFormData({
                            ...formData,
                            recurrence: {
                              ...formData.recurrence,
                      ...
                      <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '5px' }}>
                          {[...Array(31)].map((_, dayIndex) => {
                            ...
                            const isSelected = formData.recurrence.monthDays.includes(day);
                            return (
                              <div
                                key={day}
                                onClick={() => handleRecurrenceToggle(day, 'monthDays')}
                                style={{
                                  padding: '10px',
                                  textAlign: 'center',
                                  cursor: 'pointer',
                    ...
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '5px' }}>
                        {['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'].map((month, monthIndex) => {
                          const isSelected = formData.recurrence.yearMonths.includes(monthIndex + 1);
                          return (
                            <div
                              key={month}
                              onClick={() => handleRecurrenceToggle(monthIndex + 1, 'yearMonths')}
                              style={{
                                padding: '10px',
                                textAlign: 'center',
                                cursor: 'pointer',
