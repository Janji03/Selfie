...
const EventInfo = ({ selectedEvent, selectedOccurrence, handleEditEvent, handleDeleteEvent }) => {
    ...
    try {
      ...
      if (options.byweekday) {
        ...
        const days = options.byweekday
          ...
          const isWeekdayPattern = days.length === 5 && ["MO", "TU", "WE", "TH", "FR"].every(d => days.some(day => day.includes(fullDayNames[d])));
          const isWeekendPattern = days.length === 2 && ["SA", "SU"].every(d => days.some(day => day.includes(fullDayNames[d])));
      ...
      if (options.until) {
        const endDate = DateTime.fromJSDate(options.until).toLocaleString(DateTime.DATE_FULL);
  ...
  return (
    <div>
      ...
      {selectedEvent.allDay && <p><strong>All Day Event</strong></p>}
